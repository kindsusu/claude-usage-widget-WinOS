...
- 작업표시줄 왼쪽 클릭은 더 이상 데스크톱 표시를 직접 토글하지 않는다 — 표시 패널(일반/미니/숨기기 3택1 + 작업표시줄 표시 체크)을 열고 닫을 뿐이다(8번 항목 참고). `desktop_restore_mode`는 트레이 아이콘의 Show/Hide 복원(`show_widget`)에서 마지막 일반/미니 모드로 되돌리는 데 쓰인다.
...
## 8. 작업표시줄 위치 이동 — 코덱스 계약 이식 (2026-09-15)

작업표시줄 스트립의 드래그 이동·우클릭 "작업표시줄 위치" 메뉴(주/보조 모니터 × 왼쪽/오른쪽)와 보조 호스트 폴백은 자매 코덱스 위젯에서 먼저 확정한 계약을 그대로 이식했다(`SHARED STRIP CONTRACT`). `_tb_place_widget`/`_tb_choose_host`/`_tb_decide_drop`가 코덱스의 `place_taskbar_widget`/`choose_host`/`decide_drop`을 미러링하는 순수 함수이며, 존 판정(폭 절반)·보조 모니터 폴백이 설정을 바꾸지 않는 이유 등 설계 근거는 코덱스 쪽 개발 노트 13~14번 항목을 따른다. 두 위젯 중 하나만 고치면 계약이 깨지므로, 이 영역을 건드릴 때는 항상 코덱스 소스와 대조한다.

형제 순서 교체는 최종 ghost 사각형이 같은 host/zone의 형제 중심을 넘었는지로 판정하므로 형제 전체 영역과 그 너머 빈 공간까지 유효하다. 전환 중 실제 HWND는 이동 전에 중앙 렌더 경로에서 alpha 0으로 고정하고, 공유 순서 메시지와 observer wake로 양쪽 priority와 geometry를 갱신한다. fresh target이 기대 placement와 일치하고 live sibling과 겹치지 않을 때만 다시 표시한다. update·hover·cached layout도 이 gate를 우회할 수 없으며, Esc 또는 2초 timeout은 출발 사각형과 이전 priority를 복원한다.

같은 시기에 사용자 커밋 `7f67cb0`으로 표시 패널(좌클릭으로 열고 닫는 일반/미니/숨기기 3택1 + 작업표시줄 표시 체크)이 병합되어, 작업표시줄 왼쪽 클릭의 동작이 "데스크톱 표시 토글"에서 "패널 열기/닫기"로 바뀌었다. 우클릭 메뉴의 "미니 모드 (배터리)" 체크버튼도 이때 "데스크톱 위젯" 하위 메뉴(일반/미니/숨기기)로 대체됐다.
